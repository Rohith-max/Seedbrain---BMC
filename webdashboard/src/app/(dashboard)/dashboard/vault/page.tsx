...
import {
  Search, Upload, ShieldCheck, FileText, X, CheckCircle2,
  AlertCircle, Clock, Fingerprint, Banknote,
  ShieldCheck as InsuranceIcon, Stethoscope, Home,
  ...
  return (
    <div className="bg-nidhi-card border border-nidhi-border-subtle rounded-xl px-3 py-2 text-xs shadow-xl">
      ...
      {payload.map((p: any, i: number) => (
        <p key={i} style={{ color: p.fill || p.color || '#D4AF37' }} className="font-semibold">
...
function DocumentAnalytics({ docs }: { docs: any[] }) {
  ...
  docs.filter(d => d.expiryDate).forEach(d => {
    const yr = new Date(d.expiryDate).getFullYear().toString();
    yearCounts[yr] = (yearCounts[yr] ?? 0) + 1;
  });
  const expiryData = Object.entries(yearCounts)
    .sort(([a], [b]) => +a - +b)
    .map(([year, count]) => ({ year, count }));

  // Verification stats
  const verified   = docs.filter(d => d.isVerified).length;
  const unverified = docs.length - verified;
  const verifiedPct = docs.length ? Math.round((verified / docs.length) * 100) : 0;
  const verifiedArc = (verifiedPct / 100) * 2 * Math.PI * 28;
  const fullArc = 2 * Math.PI * 28;

  // File size by category (in KB)
  const sizeData = Object.entries(
    docs.reduce((acc: Record<string, number>, d) => {
      ...
      acc[cat] = (acc[cat] ?? 0) + Math.round((d.fileSize ?? 0) / 1024);
      return acc;
    }, {})
  ).map(([name, kb]) => ({ name, kb, fill: getCategoryColor(name.toLowerCase()) }));
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-left">
            <p className="text-sm font-semibold text-nidhi-text">Document Analytics</p>
            <p className="text-[11px] text-nidhi-text-muted">Category breakdown, expiry, verification & size</p>
          </div>
        </div>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-1">
              ...
              <div className="card p-6 rounded-2xl space-y-4">
                <div>
                  <p className="text-sm font-semibold text-nidhi-text">By Category</p>
                  <p className="text-xs text-nidhi-text-muted">{docs.length} total documents</p>
                </div>
                <div className="flex items-center gap-6">
                  <ResponsiveContainer width="45%" height={220}>
                    <PieChart>
                      <Pie data={categoryData} dataKey="value" nameKey="name"
                        cx="50%" cy="50%" innerRadius={52} outerRadius={82} strokeWidth={0}>
                        {categoryData.map((d, i) => <Cell key={i} fill={d.fill} fillOpacity={0.85} />)}
                      </Pie>
                  ...
                  <div className="grid grid-cols-1 gap-2 flex-1">
                    {categoryData.map((d, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="font-bold text-nidhi-text">{d.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="card p-6 rounded-2xl space-y-4">
                <div>
                  <p className="text-sm font-semibold text-nidhi-text">Expiry Timeline</p>
                  <p className="text-xs text-nidhi-text-muted">Documents expiring by year</p>
                </div>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={expiryData} barSize={40}>
                    <XAxis dataKey="year" tick={{ fontSize: 12, fill: '#9A9A8E' }} axisLine={false} tickLine={false} />
                    <YAxis hide />
                    <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
                    <Bar dataKey="count" radius={[8, 8, 0, 0]} fill="#D4AF37" fillOpacity={0.85} />
                  </BarChart>
                </ResponsiveContainer>
              </div>

              {/* Verification Rate */}
              <div className="card p-6 rounded-2xl space-y-4 flex flex-col">
                <div>
                  <p className="text-sm font-semibold text-nidhi-text">Verification Rate</p>
                  <p className="text-xs text-nidhi-text-muted">{verified} of {docs.length} verified</p>
                </div>
                  ...
                  <div className="relative">
                    <svg width={160} height={160}>
                      ...
                      <circle cx={80} cy={80} r={60} fill="none"
                        ...
                        strokeDasharray={`${(verifiedPct / 100) * 2 * Math.PI * 60} ${2 * Math.PI * 60}`}
                        strokeDashoffset={(2 * Math.PI * 60) / 4}
                        style={{ transition: 'stroke-dasharray 0.8s ease' }}
                      />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-bold text-nidhi-text">{verifiedPct}%</span>
                      <span className="text-xs text-nidhi-text-muted mt-1">verified</span>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-emerald-400">{verified}</p>
                      <p className="text-xs text-nidhi-text-muted">Verified</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-nidhi-text-muted">{unverified}</p>
                      <p className="text-xs text-nidhi-text-muted">Pending</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="card p-6 rounded-2xl space-y-4">
                <div>
                  <p className="text-sm font-semibold text-nidhi-text">Size by Category</p>
                  <p className="text-xs text-nidhi-text-muted">Storage usage in KB</p>
                </div>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={sizeData} layout="vertical" barSize={20}>
                    <XAxis type="number" hide />
                    <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: '#9A9A8E' }} width={68} axisLine={false} tickLine={false} />
                    <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
                    <Bar dataKey="kb" radius={[0, 8, 8, 0]}>
                      {sizeData.map((d, i) => <Cell key={i} fill={d.fill} fillOpacity={0.85} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
...
export default function VaultPage() {
  ...
  return (
    <div className="min-h-screen px-8 py-10 max-w-5xl mx-auto space-y-10">
      ...
      {/* ── Document Analytics Add-on ── */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <DocumentAnalytics docs={allDocs} />
      </motion.div>

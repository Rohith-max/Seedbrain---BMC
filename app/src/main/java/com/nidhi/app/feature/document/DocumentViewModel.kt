...
class DocumentViewModel(
        ...
        viewModelScope.launch {
            ...
            when (ocrResult) {
                is Result.Success -> {
                    ...
                    sendWhatsAppNotification(docTitle.ifBlank { docType }, summary.expiryDateStr)
                }
                is Result.Error ->
                    _captureUiState.update {
    ...
    private fun sendWhatsAppNotification(docTitle: String, expiryDateStr: String?) {
        try {
            val message = if (!expiryDateStr.isNullOrBlank()) {
                "📄 *$docTitle* saved in NIDHI vault\n\n⏰ Expires: *$expiryDateStr*\n\nKeep your documents organized and never miss an expiry! 🙏"
            } else {
                "📄 *$docTitle* successfully saved in NIDHI vault\n\nYour documents are safe and secure! 🙏"
            }
            
            // Send to WhatsApp (no specific number — generic share)
            WhatsAppNotifier.send(context, message)
            Toast.makeText(context, "Notification sent to WhatsApp", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
